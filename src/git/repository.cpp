...
namespace giti
{
    namespace git
    {
        ...
        public:
            ...
            FileStatus convertGitStatus(const git_status_entry *entry)
            {
                ...
                if ((status & GIT_STATUS_WT_DELETED) != 0)
                    return FileStatus::Deleted;
                if ((status & GIT_STATUS_INDEX_DELETED) != 0)
                    return FileStatus::DeletedStaged;
        ...
        std::vector<FileStatusEntry> Repository::getStatus() const
        {
            ...
            try
            {
                ...
                for (size_t i = 0; i < count; i++)
                {
                    ...
                    {
                        status_entry.filepath = entry->head_to_index->old_file.path;
                    }
                    else if (entry->index_to_workdir)
                    {
                        status_entry.filepath = entry->index_to_workdir->old_file.path;
                    }
        ...
        std::vector<FileStatusEntry> Repository::getDeletedFiles() const
        {
            auto allStatus = getStatus();
            std::vector<FileStatusEntry> deleted;

            std::copy_if(allStatus.begin(), allStatus.end(),
                         std::back_inserter(deleted),
                         [](const FileStatusEntry &entry)
                         {
                             return entry.status == FileStatus::Deleted ||
                                    entry.status == FileStatus::DeletedStaged;
                         });
        ...
        bool Repository::commit(const CommitOptions &options)
        {
            ...
            try
            {
                ...
                for (const auto &file : options.files)
                {
                    ...
                    {
                        if (status.filepath == file &&
                            (status.status == FileStatus::Deleted ||
                             status.status == FileStatus::DeletedStaged))
                        {
                            is_deleted = true;
                            break;
                        }
                    ...
                    // Use remove for deleted files, add for others
                    int result = is_deleted ? git_index_remove_bypath(index, file.c_str()) : git_index_add_bypath(index, file.c_str());

                    if (result != 0)
                    {
                        throw std::runtime_error("Failed to update index for file: " + file);
                    }
                }
